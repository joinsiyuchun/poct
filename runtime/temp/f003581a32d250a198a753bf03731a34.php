<?php /*a:2:{s:79:"/Applications/MAMP/htdocs/operator/application/index/view/settlement/index.html";i:1583564470;s:76:"/Applications/MAMP/htdocs/operator/application/index/view/public/layout.html";i:1582011447;}*/ ?>
<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
...
<script type="text/html" id="toolbar">
    <div class="dataToolbar">
        <div class="layui-inline">
            <input class="layui-input" name="keywords" id="keywords" value="<?php echo input('keywords'); ?>" autocomplete="on" placeholder="请输入工单ID">
        </div>
        <button class="layui-btn search-btn" data-type="reload"><i class="iconfont">&#xe679;</i> 查询</button>
        <?php if((buttonCheck('index/settlement/add'))): ?>
        <div class="layui-inline">
...
<script type="text/html" id="barTool">
    <?php if((buttonCheck('index/settlement/edit'))): ?>
    <a href="javascript:;" lay-event="edit" class="layui-btn layui-btn-xs"><i class="iconfont">&#xe7e0;</i> 编辑</a>
    <?php endif; if((buttonCheck('index/settlement/del'))): ?>
    {{# if(d.id == '1') { }}
    ...
    {{# } }}
    <?php endif; if((buttonCheck('index/settlement/assign'))): ?>
    {{# if ( d.id == '1' ) { }}
...
<script>
    layui.use(['jquery', 'layer', 'form', 'table'], function () {
        ...
        table.render({
            elem : '#dataTable'
            ,url : '<?php echo url("settlementList"); ?>'
            ,cellMinWidth: 80
            ,page: {
                prev: '上一页',
                next: '下一页',
        ...
        table.on('tool(dataTable)', function (obj) {
            ...
            if ( e === 'edit' ) {
                var index = layer.open({
                    ...
                    area: ['500px', '400px'],
                    content: ['<?php echo url("settlement/edit"); ?>?id=' + data.id, 'yes'],
                ...
                layer.confirm('您确定要删除吗？', {
                    ...
                    $.ajax({
                        url: '<?php echo url("settlement/del"); ?>',
                        type: 'POST',
                        dataType: 'json',
                        data: {
                            id : data.id
                ...
                var index = layer.open({
                    ...
                    area: ['400px', '625px'],
                    content: ['<?php echo url("settlement/assignWorkorder"); ?>?id=' + data.id, 'yes'],
                    skin: 'layui-layer-molv',
                    btn: ['保存', '取消'],
                    btnAlign: 'c',
                    yes: function(index, layero){
        ...
        $('#add').on('click', function () {
            var index = layer.open({
                ...
                area: ['500px', '400px'],
                content: ['<?php echo url("settlement/add"); ?>', 'yes'],
                skin: 'layui-layer-molv',
                btn: ['立即提交', '重置'],
                btnAlign: 'c',
                yes: function(index, layero){
    ...
    function setStatus(id, status) {
        ...
        $.ajax({
            url: "<?php echo url('settlement/setStatus'); ?>",
            type: 'POST',
            dataType: 'json',
            data: {
                id : id,
