...
use app\index\common\model\Measurelog as MeasureLogModel;
use think\facade\Session;

class Measurelog extends Base
{
    // 登录日志首页
    // 管理员管理首页
    public function index()
    {
        $this -> view -> assign('title', '计量日志管理');
    ...
    public function logList()
    {
        ...
        $logList = MeasureLogModel::where($map)
            -> order('id', 'desc')
            -> page($page, $limit)
            -> select();
    ...
    public function delete()
    {
        if ( Request::isAjax() ) {
            ...
            try {
                $id = Request::param('id');
                MeasureLogModel::where('id', $id) -> delete();
            } catch (\Exception $e) {
