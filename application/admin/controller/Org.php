...
class Org extends Base
{
    ...
    public function orgList()
    {
        ...
        $orgList = OrgModel::where($map)
            -> order('sort', 'asc')
            -> field('id, name as title,  pid, sort,  status, create_time, update_time')
            -> select();
//            -> toArray();
        $total = count($orgList);
        $result = array("code" => 0, "count" => $total, "data" => $orgList);
        return json($result);
    }
    ...
    public function add()
    {
        ...
        $parentorg = Request::param('parentorg');
    ...
    public function edit()
    {
        ...
        // 根据节点id查询要更新的节点信息
        $orgInfo = OrgModel::where('id', $orgId) -> find();

        // 根据父ID获取父节点名称
        if ( $orgInfo['pid'] == 0 ) {
            $parentorg = '顶级节点';
        } else {
            $parentorg = orgModel::where('id', $orgInfo['pid']) -> field('name') -> find()['name'];
        }

        // 设置模板变量
        $this -> view -> assign('title', '编辑节点');
        $this -> view -> assign('orgInfo', $orgInfo);
        $this -> view -> assign('parentorg', $parentorg);
    ...
    public function doEdit()
    {
        ...
        // 执行编辑操作
        try {
            $org = OrgModel::where('name', $data['name']) -> where('id', '<>', $data['id']) -> find();
            if ( !empty($org)) {
                return resMsg(-1, '节点名称已经存在，请重新修改', 'edit');
            }
            OrgModel::update($data);
        } catch (\Exception $e) {
